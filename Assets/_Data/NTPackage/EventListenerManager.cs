...
namespace NTPackage.EventDispatcher
{
    public enum EventCode
    {
        ...
        ForestGame_PlayerJoin,
        ForestGame_PlayerLeave,
        ForestGame_PlayerUpdate,
        ForestGame_BlockUpdate,
        ForestGame_BlockRemove,
        ForestGame_MonsterJoin,
        ForestGame_MonsterLeave,
        ForestGame_MonsterUpdate,
        GuessNumber_PlayerJoin,
        GuessNumber_PlayerLeave,
        MercaWorld_LandPotRentUpdate,
        LoadingUI_DoneLoad,
        GuessNumber_RoomLeave,
        IAP_PurchaseSuccess,
        PackageIAP_Purchase,
    }
    ...
    public class EventListenerManager : MonoBehaviour
    {
        ...
        {
            NTLog.LogMessage(key + " Register " + eventCode.ToString());
            NTDictionary<string, Action<object>> actions = this.ActionsDictionary.Get(eventCode.ToString());
            if (actions == null)
            {
        ...
        public void RemoveListener(EventCode eventCode, string key)
        {
            ...
            actions.Remove(key);
            if (actions.Dictionary.Count == 0)
            {
                this.ActionsDictionary.Remove(eventCode.ToString());
            }
        }
    }
}
